...
class Login extends Component {
  ...
  addspan = (tt) => {
    return(
      <span className = "spanlit" style = {{animation:`xs linear 1.6s infinite ${tt}s`}}></span>
    )
  }
  rendiv = () =>{
    return(
      <div className = "spnifdiv">
        {this.creanumar(15).map( (a) =>(
          this.addspan(0.48+a*0.12)
        ))}
      </div>
    )
  }
  rendivshe = (to,le,yc = 0,cln = "dottty",ds="5S") => {
    return(
      <div className = {cln} style ={{top:to ,left:le,animation: `${cln==="dottty" ?"dot":"doc"} ${ds} ${yc} infinite step-start both`}}></div>
    )
  }
  creanumar =(num)=>{
    let arr = [] ;
           ...
           {[this.rendiv()]}
            ...
            <div className="login_pro">
              ...
              <div className="loginFrame">
                {/* 四个边角 */}
                <div className="logbeforel"></div>
                <div className="logbeforer"></div>
                ...
                <Form className="logoForm" onSubmit={this.hanleSubmit}>
                  <Form.Item>
                    {getFieldDecorator("account", {
                      rules: [
                        { required: true, message: "请输入用户名!",help:"" }
                      ]
                    })(<Input className="iphoneInput" placeholder="用户名"
                    prefix={<Icon type="user"  />} />)}
                  </Form.Item>
                  <Form.Item>
                    {getFieldDecorator("password", {
                      rules: [{ required: true, message: "请输入密码!",help:"" }]
                    })(<Input className="iphoneInput" type="password" placeholder="密码"
